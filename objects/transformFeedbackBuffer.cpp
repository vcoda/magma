...
TransformFeedbackIndirectBuffer::TransformFeedbackIndirectBuffer(std::shared_ptr<Device> device, VkDeviceSize size,
    std::shared_ptr<Allocator> allocator /* nullptr */, VkBufferCreateFlags flags /* 0 */, const Sharing& sharing /* default */):
    Buffer(std::move(device), size,
        VK_BUFFER_USAGE_TRANSFORM_FEEDBACK_BUFFER_BIT_EXT | VK_BUFFER_USAGE_INDIRECT_BUFFER_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
        flags, sharing, std::move(allocator))
{}
