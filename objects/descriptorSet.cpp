...
void DescriptorSet::writeDescriptor(uint32_t index, std::shared_ptr<const AccelerationStructure> accelerationStructure)
{
    ...
    accelerationStructureDescriptorWrite.accelerationStructureCount = binding.descriptorCount;
    accelerationStructureDescriptorWrite.pAccelerationStructures = &accelerationStructures.back();
    accelerationDescriptors.push_back(accelerationStructureDescriptorWrite);
    VkWriteDescriptorSet descriptorWrite;
    descriptorWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
